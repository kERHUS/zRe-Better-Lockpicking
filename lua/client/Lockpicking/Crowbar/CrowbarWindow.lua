...
function CrowbarWindow:ActionKeyPressed()
    ...
    if arrow_scale_step >= shiftGreen/3 and arrow_scale_step <= (shiftGreen/3 + greenSize) then
		...
		self.character:getStats():setEndurance(endurance - (25 * jojo / strength + ZombRand(0, 3))/100)
		crowbarTimer = 50
    ...
    elseif arrow_scale_step >= shiftYellow/3 and arrow_scale_step <= (shiftYellow/3 + yellowSize) then
		...
		self.character:getStats():setEndurance(endurance - (32 * jojo / strength + ZombRand(1, 4))/100)
		crowbarTimer = 50
	
    else
        progressBarVal = progressBarVal + 0.04
		
		self.character:getStats():setEndurance(endurance - (40 * jojo / strength + ZombRand(2, 5))/100)
		crowbarTimer = 50
    end
